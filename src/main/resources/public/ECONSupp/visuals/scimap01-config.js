    ...
    scimap01.p.then(function() {
        ...
        function updateNodeSizeLegend(scale) {
            ...
            nodeSize.updateTextFromFunc(function(d) {
                return (scale ? scale.invert(d / 2) : 1) / scope.zoom.scale();
            });
        ...
        function applySVGEvents() {
            ...
            scimap01.SVG.underlyingNodeG.on("click.legend", function(d, i) {
                if (!d.disabled) {
                    ...
                    $.ajax({
                        type: 'GET',
                        url: econSuppServiceBase + '/journal_list?computed_rank=20&subd_id=' + d.subd_id + '&metric_id=' + metric_ids.join(',') + '&limit=20' + '&pub_year_min=' + sliderRange[0] + '&pub_year_max=' + sliderRange[1],
